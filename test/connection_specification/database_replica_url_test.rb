...
class DatabaseReplicaUrlTest < Minitest::Test
  ...
  test "use standard config/database.yml spec when DATABASE_REPLICA_URL is not defined" do
    ENV.stub :[], nil, 'DATABASE_REPLICA_URL'  do
      ...
      s.define_singleton_method(:base_config) do
        { replica: test_config }
      end
  ...
  test "use url components when DATABASE_REPLICA_URL is defined" do
    ...
    test_replica_url = "postgresql://#{test_config[:username]}:#{test_config[:password]}@#{test_config[:host]}:#{test_config[:port]}/#{test_config[:database]}?prepared_statements=#{test_config[:prepared_statements]}&pool=#{test_config[:pool]}&reconnect=#{test_config[:reconnect]}"
