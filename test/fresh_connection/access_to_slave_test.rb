...
class AccessToSlaveTest < Minitest::Test
  ...
  test "select from User is to access to slave1" do
    data = [
      @user.name,
    ...
    assert data.all?{|n| n.include?("slave1")}
  end

  test "select from Address is to access to slave1" do
    data = [
  ...
  test "select from Address is to access to slave2" do
    data = [
      Tel.first.number,
      @user.tels.first.number
    ]

    assert data.all?{|n| n.include?("slave2")}
  end

  test "select with join is to access to slave1" do
    name = User.joins(:address).where("addresses.user_id = 1").where(id: 1).first.name
    assert_includes name, "slave1"
  end

  test "pluck is access to slave1" do
    assert_includes User.where(id: 1).pluck(:name).first, "slave"
  end
  ...
  test "count is access to slave" do
